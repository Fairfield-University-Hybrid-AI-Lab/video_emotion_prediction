...
def main():
    ...
    frame_number = 0
    cap = cv2.VideoCapture("\\Users\\George\\Documents\\Python\\ADS CapStone\\aff_wild_annotations_bboxes_landmarks_new\\videos\\train\\"+str(VIDEO_NUMBER)+".avi")
    while(cap.isOpened()):
        frame_number += 1
        ...
        fullarraypts = np.empty([2,range(len(bounding_box))])
        for x in bounding_box:
            np.append(fullarraypts,[x[0],x[1]])
